...
def execute_query_safely(conn, query: str, target_db: str, max_retries: int = 3, timeout_seconds: int = 10) -> Tuple[bool, str, float]:
    """
    쿼리를 안전하게 실행합니다.
...
def test_workload_file(workload_file: str, target_db: str, max_queries: int = None, save_successful_only: bool = False, query_timeout: int = 10) -> Dict:
    ...
    for i, query_data in enumerate(queries):
        ...
        # 쿼리 실행
        success, error_msg, exec_time = execute_query_safely(conn, sql_query, target_db, max_retries=3, timeout_seconds=query_timeout)
        ...
        if success:
            ...
            if save_successful_only:
                results["successful_queries_data"].append(query_data)
            if (i + 1) % 50 == 0:
...
def save_successful_workload(original_file_path: str, target_db: str, successful_queries: List[Dict], output_dir: str):
    """성공한 쿼리들만으로 새로운 워크로드 파일을 생성합니다."""
    import os
    ...
    # 마스킹 개수 분포 계산
    masking_distribution = calculate_masking_distribution(successful_queries)
    ...
    # 원본 파일 경로에서 상대 경로 추출
    workloads_base = "/data/yhyunjun/HybridSQL-Benchmark/workload-construction-2/data/workloads"
    ...
    success_rate = (successful_count / original_count * 100) if original_count > 0 else 0
    
    print(f"💾 성공한 쿼리 {successful_count}/{original_count}개 ({success_rate:.1f}%) 저장: {output_file_path}")
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, 
                       default='/data/yhyunjun/HybridSQL-Benchmark/workload-construction-2/data/workloads_v3',
    ...
    workloads_dir = "/data/yhyunjun/HybridSQL-Benchmark/workload-construction-2/data/workloads"
    ...
    for file_path, target_db in filtered_files:
        ...
        # 성공한 쿼리들 저장 (옵션이 활성화된 경우)
        if args.save_successful and "successful_queries_data" in result and result["successful_queries_data"]:
            save_successful_workload(file_path, target_db, result["successful_queries_data"], args.output_dir)
